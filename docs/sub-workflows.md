...
In some cases, a sub-workflow will require data that is generated by a different sub-workflow as a dependency.  In these events, the required processes will be automatically activated by TEMPO.  For example, output from the `mutsig` process is required for the `snv` sub-workflow to function.  Specifying `workflow="mutsig"` will then automatically enable the `snv` processes to accomidate the dependency.
