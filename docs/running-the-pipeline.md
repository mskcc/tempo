...
* `--workflows` inidicates which [sub-workflows](sub-workflows.md) should be executed for this run. Possible options are `snv`, `sv`, `mutsig`, `germSNV`, `germSV`, `hlaLoH`, `facets`,`qc`,`hlatyping`,`neoantigen`, and `msisensor`. Multiple arguments can be provided in quotation marks (i.e. `--workflows="snv,qc"`). 
