...
* `--abra2` enables [ABRA2](https://github.com/mozack/abra2) assembly-based indel realignment of the aligned BAMs, performed after MarkDuplicates and before BQSR. For `--assayType exome` the capture kit `targets.bed` restricts realignment; for `--assayType genome` it runs genome-wide and is currently untested at that scale (expect to tune ABRA2 CPU/memory/walltime). Default: `false`.
