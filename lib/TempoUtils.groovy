  ...
  def watchMapping(tsvFile, assayType) {
    Channel.watchPath( params.watchMapping, 'create, modify' )
        ...
        .map{ row ->
                ...
                if(!checkTarget(target, assayType)){}
                if(!checkNumberOfItem(row.split("\t"), 5, tsvFile)){}
  ...
  def watchBamMapping(tsvFile, assayType){
    Channel.watchPath( params.watchBamMapping, 'create, modify' )
        ...
        .map{ row ->
                ...
                if(!checkTarget(target, assayType)){}
                if(!checkNumberOfItem(row.split("\t"), 4, tsvFile)){}
  ...
  def watchPairing(tsvFile){
    Channel.watchPath( params.watchPairing, 'create, modify' )
           ...
           .map { row ->
                ...
                if(!checkNumberOfItem(row.split("\t"), 2, tsvFile)){}
