process SVTOOLS_VCF2BEDPE_GERMLINE {
    tag "${meta.id}"
    label 'process_low'
    ...
    echo -e "${sample_id} NORMAL" > normalize.samplenames.tsv
    bcftools reheader \\
    ...
    if [ ! -s ${prefix}.combined.tmp.bedpe ] ; then
        echo -e "#CHROM_A\\tSTART_A\\tEND_A\\tCHROM_B\\tSTART_B\\tEND_B\\tID\\tQUAL\\tSTRAND_A\\tSTRAND_B\\tTYPE\\tFILTER\\tNAME_A\\tREF_A\\tALT_A\\tNAME_B\\tREF_B\\tALT_B\\tINFO_A\\tINFO_B\\tFORMAT\\tNORMAL" >> ${prefix}.combined.tmp.bedpe
    fi

    zgrep "^##" ${vcf} | sed "s/##fileformat=*/##fileformat=BEDPE/g" > ${prefix}.combined.unsorted.bedpe
    grep -v "^##" ${prefix}.combined.tmp.bedpe | \\
        awk -F"\\t" -v nid="${sample_id}" -v OFS="\\t" \\
        'NR == 1 {print \$0,"NORMAL_ID";next;}{print \$0,nid}' \\
