  ...
  process AlignReads {
    ...
    input:
      tuple val(idSample), val(target), file(fastqFile1), val(sizeFastqFile1), file(fastqFile2), val(sizeFastqFile2), val(fileID), val(lane)
      tuple file(genomeFile), file(bwaIndex)

    output:
      tuple val(idSample), file("*.html"), emit: fastPHtml
      tuple val(idSample), file("*.json"), val(fileID), emit: fastPJson4MultiQC
      path("file-size.txt"), emit: laneSize
      tuple val(idSample), val(target), file("*.sorted.bam"), val(fileID), val(lane), file("*.readId"), emit: sortedBam
